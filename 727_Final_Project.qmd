---
title: "Capstone New Direction"
format: html
editor: visual
Title: Text Based Analysis of Measles Outbreak Narratives in CDC Reports and Their Relationship With State Demographic Patterns in the United States
---

## **SUMMARY OF PROPOSAL**

This project will analyze how measles outbreaks are described across official CDC reports and explore how these outbreak patterns relate to demographic characteristics of US states. Instead of examining public sentiment, the study focuses on public health reporting patterns, extracting state level outbreak mentions from CDC text and comparing these geographic patterns with demographic variables from the American Community Survey (ACS). The goal is to understand which states appear most frequently in outbreak narratives, what themes emerge from CDC reporting, and whether demographic factors help explain the distribution of outbreaks across states.
...
1.  Which US states are most frequently mentioned in CDC measles outbreak reports, and what does this reveal about geographic patterns of measles activity?

2.  What major themes and risk factors (such as vaccination gaps, travel importation, or demographic vulnerabilities) appear in CDC outbreak narratives based on text analysis?

3.  How do state level demographic characteristics from the ACS relate to outbreak representation in CDC reports, and can demographic patterns help explain why some states appear more frequently than others?
...
### Extract Outbreak Relevant Blocks
...
state_regex <- str_c("\\b(", str_c(states, collapse = "|"), ")\\b")
...
### Checking New York vs. New York City Mentions

```{r}
nyc_count <- outbreak_cdc %>%
  filter(str_detect(text, regex("new york city", ignore_case = TRUE))) %>%
  tally()

ny_state_only <- outbreak_cdc %>%
  filter(
    str_detect(text, regex("new york", ignore_case = TRUE)) &
    !str_detect(text, regex("new york city", ignore_case = TRUE))
  ) %>%
  tally()

ny_total <- outbreak_cdc %>%
...
merged_data %>%
  ...
  coord_flip() +
  labs(
    title = "Top 15 States Mentioned in CDC Measles Outbreak Text",
    x = "State",
...
Larger states appear more frequently in CDC measles reports, reflecting greater population size and higher likelihood of case reporting.

### Mentions per Million People
...
merged_data %>%
  ...
  coord_flip() +
  labs(
    title = "Top 15 States by Outbreak Mentions per Million Residents",
    x = "State",
    y = "Mentions per Million People"
  )

```

When adjusted for population, some smaller states show higher outbreak reporting intensity, suggesting that population alone does not fully explain outbreak patterns.
...
Larger states tend to appear more often in CDC measles reports. The upward trend shows that population size is a major factor in how frequently a state is mentioned.
...
There is little to no relationship between education level and outbreak mentions. The trend is nearly flat, suggesting education does not strongly influence how often a state appears in CDC reports.

### Prepare Text for Topic Modelling

#### Topic modelling tokinization.
...
CDC outbreak text included references to multiple states, with some states appearing more frequently than others. Raw mention counts showed that larger states tended to appear more often, consistent with their larger population sizes and higher likelihood of reporting measles cases. After merging with ACS data, a normalized metric (mentions per million residents) was calculated to allow fair comparison across states of different sizes.

### Demographic Comparison

When comparing CDC outbreak mentions with ACS demographic variables, a clear positive relationship was observed between state population and the number of CDC mentions. States with larger populations generally had more outbreak references. However, the relationship between outbreak mentions and education level was weak, indicating that educational attainment did not strongly influence how often a state appeared in CDC outbreak reporting.
...
These topics indicate that CDC communications emphasize a mix of national level reporting, surveillance updates, public health guidance, and specific outbreak details.

### Normalized Outbreak Patterns

After adjusting for population, some smaller states showed higher outbreak mentions per million residents. This indicates that population size alone does not fully explain outbreak reporting patterns. The normalized measure offers a clearer view of outbreak intensity relative to population and adds context beyond raw mention counts.
